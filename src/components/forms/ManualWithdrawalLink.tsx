...
interface Props {
  platformTxId: string      // DB id of the platform discrepancy transaction
  platformAmount: number
  currency: string
  onResolved: () => void
}

export default function ManualWithdrawalLink({ platformTxId, platformAmount, currency, onResolved }: Props) {
  ...
  async function handleLink() {
    ...
    try {
      const res = await fetch('/api/reconciliation/resolve-withdrawal', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ platformTxId, shamCashTxId: shamCashTxId.trim() }),
      })
  ...
  return (
    <div className="flex flex-col gap-1.5">
      ...
      <p className="text-xs text-gray-400">
        المنصة: {platformAmount.toLocaleString('en', { minimumFractionDigits: 2 })} {currency}
      </p>
    </div>
  )
}
